...
function Contact(id, name, address, number,mail){
    this.id = id;
    this.name = name;
...
addBtn.addEventListener('click', function(){
    if(checkInputFields([name, address, number,mail])){
        setMessage("success", "Record added successfully!");
        id++;
        const contact = new Contact(id, name.value, address.value, number.value,mail.value);
...
// Adding to List (on the DOM)
    function addToList(item){
        ...
        <div class = "record-el">
            ...
            <span id = "contact-num-content">${item.mail}</span>
        </div>
